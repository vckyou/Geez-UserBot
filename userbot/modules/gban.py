...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        ...
        dark = await dc.edit("`Memproses Global Banned Si Jamet!!`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Si Jamet Akan Segera Aktif Tuan!!`")
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Membatalkan Perintah Global Banned Si Bocah Kampung`")
    else:
        dark = await dc.edit("`Sabar Goblok Lagi Gua Proses`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Memulai Proses Ungban Si Bocah Kampung`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    a = b = 0
