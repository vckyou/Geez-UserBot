...
@register(outgoing=True, pattern=r"^\.autopic (.*)")
...
@register(outgoing=True, pattern=r"^\.digitalpfp (.*)")
...
@register(outgoing=True, pattern=r"^\.bloom (.*)")
async def autopic(event):
    if event.fwd_from:
        return
    if Config.DEFAULT_PIC is None:
...
@register(outgoing=True, pattern=r"^\.autoname (.*)")
async def _(event):
    if event.fwd_from:
        return
...
@register(outgoing=True, pattern=r"^\.autobio (.*)")
async def _(event):
    if event.fwd_from:
        return
